document.addEventListener("DOMContentLoaded", function () {
    ...
    function updateSummary() {
        ...
        if (contractorElement && formData.contractor) {
            contractorElement.textContent = `${formData.contractor.name} - ${formatCurrency(formData.contractorAmount)}`;
        }
        ...
        if (companyElement) {
            ...
            companyElement.textContent = `ООО "ВашаКомпания" - ${formatCurrency(totalCompanyAmount)}`;
        }
    ...
    function calculateTotal() {
        const totalItemsAmount = formData.items.reduce((sum, item) => sum + item.amount, 0);
        return formData.contractorAmount + formData.companyAmount + totalItemsAmount + formData.commission;
    }
    ...
    generateFinalBtn.addEventListener("click", function () {
        if (validateStep(currentStep)) {
            ...
            currentStep = 4; // Переходим на шаг результата
            updateProgress();

            // Здесь будет реальная интеграция с API
            const totalAmount = calculateTotal();
            const linkInput = document.querySelector(".generated-link input");
            linkInput.value = `https://pay.tinkoff.ru/split-payment-${Date.now()}?amount=${totalAmount}`;

            // Обновляем информацию в результате
    ...
    axios.get("https://module-bek.onrender.com/api/v1/contractors").then((res) => {
    ...
    axios.get("https://module-bek.onrender.com/api/v1/nomenclature").then((res) => {
