document.addEventListener("DOMContentLoaded", function () {
  ...
  // Базовый URL API
  const API_BASE_URL = "http://91.143.16.246:3000/api/v1";
  ...
  async function processPayout() {
    ...
    try {
      await axios.post("http://91.143.16.246:3000/api/v1/payment/payout", {
...
Дата создания: ${new Date(payout.createdAt).toLocaleString("ru-RU")}
  ...
  function confirmPayout(payoutId) {
    axios
      .post("http://91.143.16.246:3000/api/v1/payment/confirm", {
        paymentId: payoutId,
      })
      .then((response) => {
